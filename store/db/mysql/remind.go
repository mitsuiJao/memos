...
func (d *DB) CreateRemind(ctx context.Context, create *store.Remind) (*store.Remind, error) {
	...
	id := int32(rawID)
	list, err := d.ListReminds(ctx, &store.FindRemind{ID: &id})
	if err != nil {
		return nil, err
	}
	if len(list) == 0 {
		return nil, errors.New("failed to create remind")
	}
	return list[0], nil
}
...
func (d *DB) CreateRemindEphemeralMemo(ctx context.Context, create *store.RemindEphemeralMemo) error {
	stmt := "INSERT INTO `remind_ephemeral_memo` (`memo_uid`, `expire_ts`) VALUES (?, ?) ON DUPLICATE KEY UPDATE `expire_ts` = VALUES(`expire_ts`)"
